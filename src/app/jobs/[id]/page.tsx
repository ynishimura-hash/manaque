...
export default function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleApplyConfirm = async () => {
        ...
        try {
            ...
            // 1. Create chat with detailed initial message
            const initialMessage = `【求人への応募申し込み】
求人タイトル: ${job.title}
企業名: ${company.name}
給与・報酬: ${job.salary || job.reward || '-'}
勤務地: ${job.location || company.location || '-'}

この求人についてお話ししたいです。よろしくお願いいたします。`;
                    ...
                    <div className="p-4 md:p-8 space-y-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                ...
                                <div className="bg-zinc-50 rounded-2xl p-6 border border-zinc-100">
                                    <span className="block text-xs text-zinc-400 font-black uppercase mb-1">応募要件</span>
                                    <p className="text-sm font-bold text-zinc-700 leading-relaxed whitespace-pre-wrap">{job.requirements || '特になし'}</p>
                                </div>
                                ...
                                <div className="bg-zinc-50 rounded-2xl p-6 border border-zinc-100">
                                    ...
                                    {job.salary && (
                                        <p className="text-lg font-bold text-zinc-900 mb-2">
                                            <span className="text-xs text-blue-600 font-bold block mb-0.5">月収例</span>
                                            {job.salary}
                                        </p>
                                    )}
                                    <p className="text-sm font-medium text-zinc-700 leading-relaxed whitespace-pre-wrap">
                                        {job.reward || '経験・能力を考慮の上決定'}
                                    </p>
                                </div>
